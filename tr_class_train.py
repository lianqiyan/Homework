...
class Data(object):
    ...
    def load(self):
        ...
        if not os.path.exists(point_dir):
            ...
            # class_path = r'G:\Oxford\groundtruth'
            ...
            self.img_names = [n.encode("ascii", "ignore") for n in self.img_names]
            with h5py.File(point_dir, 'w') as hf:
                hf.create_dataset('data', data=self.images)
                hf.create_dataset('label', data=self.labels)
                # hf.create_dataset('img_names',(len(self.img_names),1),'S10', data=self.img_names)
        else:
            all_name = os.listdir(self.class_path)
            cls = []
            b_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_path', type=str, default= '/data/lianqiyan/image_search/groundtruth')
    parser.add_argument('--data_path', type=str, default= '/data/lianqiyan/image_search/images')
    ...
    cnn = CNN(config.img_size, train_data, config.channel, [32, 16, 8], [5, 3, 3], [256 128], 10000, 0.0001)
    # cnn.train()
